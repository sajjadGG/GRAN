...
class GRANMixtureBernoulli(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.att_edge_dim = 64  # TODO: fixed param
    ...
    def _inference(
        ...
        if self.dimension_reduce:
            node_feat = self.decoder_input(A_pad)  # BXCXN_max X H
        else:
    ...
    def _sampling(self, B):
        ...
        with torch.no_grad():
            ...
            A_prob = torch.zeros(B, N_pad, N_pad).to(self.device)
            ...
            if self.is_sym:
                ...
                A_prob = torch.tril(A_prob, diagonal=-1)
                A_prob = A_prob + A_prob.transpose(1, 2)

            global called_num
            with open(f"tmp/prob{called_num}.pkl", "wb") as f:
                dump(A_prob, f)
            called_num += 1
            return A
...
def mixture_bernoulli_loss(
    ...
    bc_idx = torch.repeat_interleave(bc_idx, bc_size.long().to(label.device))  # S
